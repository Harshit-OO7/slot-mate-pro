...
export default function Dashboard() {
  const { activeSession, slots, transactions } = useParking();

  const activeSlot = activeSession ? slots.find(s => s.id === activeSession.slotId && s.locationId === activeSession.locationId) : null;
  const locationName = activeSession ? LOCATIONS.find(l => l.id === activeSession.locationId)?.name : null;
  const rate = activeSession ? (LOCATIONS.find(l => l.id === activeSession.locationId)?.ratePerHour || 50) : 50;
      ...
      <div className="container mx-auto max-w-4xl">
        ...
        {activeSession && activeSlot ? (
            ...
            <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}>
              <div className="glass-panel p-6 space-y-4">
                ...
                <div className="space-y-3 pt-4 border-t border-border/50">
                  ...
                  <InfoRow label="Rate" value={`₹${rate}/hour`} />
                </div>
              </div>
            </motion.div>

            <motion.div initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }}>
              <LiveBill entryTime={activeSession.entryTime} rate={rate} />
            </motion.div>
          </div>
        ) : (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="glass-panel p-12 text-center">
            ...
            <p className="text-xs text-muted-foreground/50 mt-1">Select a location and enter a vehicle from the home page</p>
          </motion.div>
        )}
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-border/50">
                    ...
                    <th className="text-left text-[10px] font-mono text-muted-foreground uppercase tracking-wider p-3">Location</th>
                ...
                <tbody>
                  {transactions.map(tx => (
                    <tr key={tx.id} className="border-b border-border/30">
                      ...
                      <td className="p-3 text-sm text-muted-foreground">{LOCATIONS.find(l => l.id === tx.locationId)?.name || tx.locationId}</td>
